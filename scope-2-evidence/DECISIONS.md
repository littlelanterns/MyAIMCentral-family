...
- **PATTERN-FLAG-FOR-SCOPE-3: PRD-05 ↔ PRD-24A LiLa generation seam** — bingo/boss/passport generation cascading absence resolves when F60c builds kick off; cross-PRD traversal should verify LiLa context-assembler hooks land alongside the write-target tables.

### Founder adjudication

- **F58 Reward economy unbuilt — near-term lego-piece connector:** 7 of PRD-24's 8 tables absent + 6 screens absent + DailyCelebration Step 3/4 auto-skipped. Founder direction 2026-04-20: "Reward economy, that should be a lego piece as well, that can plug into any/all relevant lists, tasks, routines, best intentions, goals, etc." Shifted from F1/F9 "when paid tiers ship" pattern to F22/F34 "near-term roadmap with cross-feature composability" pattern. Remediation scope: not just Screen 3 reward menu — reward containment + treasure box + reward redemption as cross-feature connectors that plug into any task/list/routine/best-intention/goal source. Build when founder prioritizes onto active queue.

- **F59 PRD-24 pipeline + settings supersession:** Conventions #198–207 + #221 captured the architectural pivot. PRD-24 text stays with pointer amendment citing Conventions, not rewrite. F21 precedent anchors Low severity (addendum properly captured). Worker proposed Medium; orchestrator override to Low matches F21 "addendum-captured supersession does not harm more than text-stale other PRDs" reasoning.

- **F60a PRD-24A dashboard backgrounds → Sticker Book pages:** Clean subset supersession. Founder direction: "The only thing we kind of changed was the sticker book background pages instead of the background being an overlay/background for the entire dashboard." `dashboard_backgrounds` + `background_library` specifically replaced by `gamification_sticker_pages`. Conventions #198–207 capture. PRD-24A §Data Schema L1122-1139 + L1232-1252 pointer amendment. Low severity per F21/F38 precedent.

- **F60b PRD-24A overlay engine deferred indefinitely:** Founder direction: "The overlay engine I'd like to pause on, I don't want it deleted forever, but I do want it deferred as an idea to possibly implement at a much later date. Maybe never..." Distinct shape from F60c — dashboard-wide narrative game layer with 4-stage progression paused indefinitely. Sticker Book substitutes. STUB_REGISTRY add with "deferred indefinitely — possibly never" framing to distinguish from active-roadmap F60c entries. Not deleted, not near-term. Medium severity acknowledges the scope of architecture paused (3 tables: overlay_instances + overlay_collectibles + recipe_completions + entire engine concept).

- **F60c PRD-24A themes + game modes on active roadmap:** Founder direction: "All of the other themes and game modes are still needing to be built. They need to be on the roadmap. All of those." 5 launch visual themes (Pets, Apothecary Bottles, Dragons, Pixel Loot, Mythical Creatures) + 7 game modes (Daily Growth, Category Collection, Recipe & Quest, Stamp Passport, Streak Evolution, Boss Battle, Family Bingo) + Game Modes Addendum tables (boss_quests, bingo_cards, evolution_creatures, passport_books) + LiLa bingo/boss/passport generation. F22/F34 near-term-roadmap pattern. Additional founder architectural intent: Woodland Felt asset reuse across game modes (bingo theme, recipe mode, sized collection) — confirms modular Visual Theme × Game Mode per PRD-24A design. STUB_REGISTRY L239-243 Post-MVP entries upgrade to "active roadmap."

- **F61 PRD-24B reveal library needs cross-feature wiring:** Founder direction explicitly flips the "delete dead code" read to "wire it as cross-feature lego." Opportunity list every N completions → reveal, sequential list milestone → reveal, randomized consequence list → card/door/scratch-off/spinner or rotation, routine with mixed connectors (sequential item + randomized item). Classification flipped to Unintentional-Fix-Code, severity Medium. Pre-build architectural question: reward-reveals/ parallel architecture (5 tables, 33 reveal_animations rows) needs reconciliation with gamification/reveals/ — either merge into one lego library or preserve both for different purposes. Decision required before F61 build kicks off.

- **F62 PRD-24B Color Reveal needs fuller architecture:** Founder direction: "I would like color reveal to also be able to connect to multiple tasks [click all that apply] or attach to streaks, book chapters, or any type of list etc. It would again be like a lego piece connector that mom can connect anywhere to anything..." Build M Conventions #211–213 NOT final — they were baby-step simplification. Classification flipped to Unintentional-Fix-Code, severity Medium. Full architecture restores PRD-24B Screen 10 rich model (any achievement source + Sequential/Gradual/Random strategies + complexity 1-5 with 3-100 zones) AND expands to multi-task click-all-that-apply, streak milestones, book chapter completions, any list type. Conventions #211–213 amendment/retirement when F62 ships.

- **F63 PRD-24 screen features unbuilt:** Schema-ready, code-dormant. 5 visualization modes → 1 ships, `gamification_level` unused, Family Leaderboard widget generic scaffolding. F2/F16/F32 gate-adoption pattern. Fix when roadmap-relevant; surfaces naturally during F58 reward economy build.

### Emission list

Findings to emit (8 total):
- SCOPE-2.F58 — PRD-24 reward economy unbuilt — near-term lego-piece connector (Medium, Deferred-Document, Beta N; F22/F34 near-term roadmap pattern; cross-feature composability remediation scope)
- SCOPE-2.F59 — PRD-24 pipeline + settings panel superseded by Build M (Low, Intentional-Document, Beta N; F21 precedent; pointer amendment per Conventions #198–222)
- SCOPE-2.F60a — PRD-24A dashboard backgrounds → Sticker Book pages (Low, Intentional-Document, Beta N; SUPERSEDES Conventions #198–207)
- SCOPE-2.F60b — PRD-24A overlay engine deferred indefinitely (Medium, Deferred-Document, Beta N; founder "pause, maybe never"; STUB_REGISTRY entry with "deferred indefinitely" framing)
- SCOPE-2.F60c — PRD-24A themes + game modes + Game Modes Addendum tables on active roadmap (Medium, Deferred-Document, Beta N; F22/F34 near-term roadmap pattern; STUB_REGISTRY L239-243 Post-MVP → active roadmap upgrade)
- SCOPE-2.F61 — PRD-24B reveal library needs cross-feature lego wiring (Medium, Unintentional-Fix-Code [classification flip from Deferred-Document], Beta N; reward-reveals/ architecture reconciliation as pre-build question)
- SCOPE-2.F62 — PRD-24B Color Reveal needs fuller lego-connector architecture (Medium, Unintentional-Fix-Code [classification flip from Intentional-Document], Beta N; Conventions #211–213 amendment when F62 ships)
- SCOPE-2.F63 — PRD-24 screen features unbuilt (Low, Unintentional-Fix-Code, Beta N; F2/F16/F32 gate-adoption pattern)
...
2. **F61 reward-reveals/ vs gamification/reveals/ architectural reconciliation** — pre-build decision required before F61 implementation. Options: (a) merge both into unified lego reveal library; (b) preserve both for distinct purposes with documented scope boundaries; (c) retire gamification/reveals/ once reward-reveals/ expands to cover PRD-24B's 8-type flat catalog. Not Scope 2 scope to decide; flagged as F61 pre-build gate.

3. **F62 Conventions #211–213 amendment/retirement** — when F62 full lego-connector architecture ships, Conventions #211 (1:1 task-linked tally counter), #212 (4-field config), #213 (32 Woodland Felt subjects) need amendment to cite the new multi-source model. STUB_REGISTRY color-reveal entry updates from "deferred expansion" to "wired as cross-feature connector" at build completion.

4. **F60c STUB_REGISTRY L239-243 upgrade** — current entries mark Family Challenges (PRD-24C), Boss Quests, Bingo Cards, Evolution Creatures, Passport Books as "📌 Post-MVP." Per founder direction, these upgrade to "active roadmap" framing (e.g., "⏳ Unwired (MVP)" or "⏳ Roadmap"). Phase 3 registry-hygiene pass.

5. **PRD-05 ↔ PRD-24A LiLa bingo/boss/passport generation seam** — cascading absence from F60c. Resolves when F60c builds land. Scope 3+8b traversal should verify LiLa context-assembler hooks + `lila_guided_modes` rows + Edge Functions land alongside `boss_quests` + `bingo_cards` + `passport_books` table creation. Cross-PRD seam evidence captured in EVIDENCE_BATCH_8 Row PRD-24A R5.

6. **F61 `lists.reveal_type` dormant column** — column landed (migration `00000000000008_tasks_lists.sql:310`) but no consumer. Dormant until F61 wires the reveal library. Note: `lists.reveal_visual` was PRD-24 Cross-PRD's original name; PRD-24B's Cross-PRD flagged the naming reconciliation. Live schema shipped `reveal_type` — stale `reveal_visual` references in PRD-24 text should update as part of F61 remediation.

7. **F60b overlay engine "deferred indefinitely" STUB_REGISTRY framing** — distinct category from "Post-MVP" + "Active roadmap." Founder language "pause, maybe never" suggests a third bucket for ideas preserved-but-not-scheduled. Phase 3 registry-hygiene pass may warrant a new status marker.
