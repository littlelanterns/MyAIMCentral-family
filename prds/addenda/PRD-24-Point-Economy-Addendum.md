...
|---|---|---|---|
| D-PECON-1 | Config-as-truth resolution + the three pipeline repairs | | |
| D-PECON-2 | Per-routine points on the template (default none; live propagation; per-kid mode retired; shared-routine contribution rule) | | |
| D-PECON-3 | Intention points: one per-kid value, default off | | |
| D-PECON-4 | Shop shape: family catalog, per-item audience/cost/approval; Play always-approval; reward_rules grant; display name | | |
| D-PECON-5 | Spend mechanics: append-only ledger, deduct-at-purchase w/ refund, purchases → earned_prizes | | |
| D-PECON-6 | Completion-% as unlock gate (weekly window, progress-forward framing), in scope v1 | | |
| D-PECON-7 | Points-only v1; dollars + treasure boxes stubbed | | |
| D-PECON-8 | PECON supersedes RSTP (step-prize scope carried intact as its own slice) | | |
| Riders 1–4 | Per-task point values (`points_override` reused); daily points goal (optional, celebration-only, contracts-hookable); per-step shared attribution PINNED; three-independent-rails PINNED | **Founder-directed 2026-07-07** ("two additions… two more requirements, both to PIN with tests") — folded, awaiting her scope confirmation before Worker A dispatch | 2026-07-07 |
