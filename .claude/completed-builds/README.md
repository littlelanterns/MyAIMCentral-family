...
|---|---|---|---|
| Member-Day Task State — Single Source of Truth | PRD-09A/PRD-28 cross-cutting — Fixes silent allowance erosion from past-end-date painted routines. Canonical `get_member_day_obligations` (Layer 2) + `obligation_active_for_member_on_date` (Layer 1, mirrors `recurringTaskFilter.ts`); 3 callers refactored; Convention #271 codified; invariant test guards TS↔SQL drift (19/19); 3 painted rows backfilled + 1 orphan archived. Return shape pre-carries pool_id/task_segment_id/is_extra_credit/homework_subject_ids for future reward surfaces. Migration 100247. Commit `6143b5a`. 12/12 Wired, 0 Missing, 3/3 Discipline-2 prod checks. | 2026-06-08 | [member-day-task-state-canonical-source.md](2026-06/member-day-task-state-canonical-source.md) |
...
*Family-Auth-Two-Door (PRD-01/PRD-02 amendment) completed 2026-06-09. Verification archived in `family-auth-two-door.md` (this folder) + `claude/feature-decisions/Family-Auth-Two-Door.md`. 26 wired, 2 stubbed, 0 missing across 28 requirements. Closed a LIVE roster-enumeration hole (SECURITY DEFINER lookup RPCs with zero auth — anyone could harvest member names/avatars by guessing family names). Shipped in 4 phases same-day: (1) leak closure, (2) family password door — combined no-enumeration verify, bcrypt + 5/15 lockout, Settings page, forced setup modal, npm run family:password script, (3) Family identity — per-family shadow account + hidden role='family' member row (own permission scope, future hub config anchor), choice screen with Hub/personal resting places, kill switch (password change rotates shadow + global sign-out), View As member_session policies for family devices, (4) single-picture password — server-side verify with fixed decoy sets, session minting via server-derived HMAC secret, mom picker UI, hub dip-in support; PIN set/auto-assign now creates login accounts (historic FamilyMembers TODO). Migrations 100251-100254 + 100256-100258 (100255 was the parallel role-scoping session — first-ever two-session collision, resolved by renumber). family-auth-admin Edge Function (--no-verify-jwt). Convention #273. E2E suite tests/e2e/features/family-auth-two-door.spec.ts 8/8 incl. real-family PIN personal-device login and the full kill-switch pipeline. Stubs: personal-device timeout PIN-relock (two-layer stickiness completion), mobile eyes-on follow-up.*
