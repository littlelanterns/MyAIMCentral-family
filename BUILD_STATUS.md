...
| FO-COMMAND-CENTER | Family Overview Command Center | Founder vision restructure (gate-approved 2026-06-10, Q1–Q9): Family Overview becomes mom's command center — page tabs Overview/Approvals(N)/Queue(N)/Finances; routines (via `get_member_day_obligations`, #271) + sequential + victories + weekly-completion (live allowance %) column sections in founder Q5 order with read-time saved-order merge; member spot-check deep view (tap column header → founder tab set + inline complete + FULL edit modal via shared `useTaskEditor`/`TaskEditModal`); Sequential [+Create] relocated (Q8, #150 amended); Tasks page purely personal (every role, soft-claim §4.5 kept) with the 13-view carousel (one exported ViewRenderer, simple_list keeps Build M segments) + Q2 hybrid inclusion control (persisted default + day pills, sequential next-item-only with tap-through); dad granted-scoped FO via useViewableMembers (stale view_as_permissions read removed; SA never offered FO); deep links + legacy redirects. Zero migrations. Convention #275. E2E `fo-command-center.spec.ts` 10/10 + leak-pass 10/10 + permissions-wiring 14/14 (pins updated to relocated surfaces, same no-leak intents). Coordinated live with REVIEW-ROUTE-TASK-SCOPING session (assignment scoping inherited; "Deploy all" Queue button shipped same-session on their engine). Verification: 26 Wired / 2 Stubbed / 0 Missing. | **Code complete — pending founder eyes-on** | 2026-06-10 |
