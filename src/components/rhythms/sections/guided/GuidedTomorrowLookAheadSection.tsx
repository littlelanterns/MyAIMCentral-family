/**
 * PRD-18 Mini Evening Rhythm for Guided — Tomorrow Look-Ahead
 *
 * Rotating wording: 6 different framings of the same forward-looking
 * question, picked deterministically by date so the same kid sees the
 * same wording all day, a different one tomorrow. Some wordings ask
 * about anticipation ("looking forward to"), some about intention
 * ("planning on"), some about general framing ("on your mind") — same
 * function, different emotional shape.
 *
 * The selected wording is saved INTO the journal entry content for
 * historical fidelity. Tags stay constant (`tomorrow`).
 *
 * Writes directly to journal_entries with:
 *   - entry_type = 'reflection'
 *   - tags = ['reflection', 'guided_evening', 'tomorrow']
...
function useSaveTomorrowReflection() {
...
export function GuidedTomorrowLookAheadSection({ familyId, memberId, readingSupport }: Props) {
  const [text, setText] = useState('')
  const [savedAt, setSavedAt] = useState<number | null>(null)
  const save = useSaveTomorrowReflection()

  // Date-seeded rotation: same kid sees same wording all day, different tomorrow.
  const promptText =
    pickOne([...TOMORROW_PROMPTS], rhythmSeed(memberId, 'evening:tomorrow', new Date()))
    ?? TOMORROW_PROMPTS[0]
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <Sunrise size={22} style={{ color: 'var(--color-accent-deep)' }} />
        <h3
      ...
      <textarea
        ...
        placeholder="A plan, a person, a snack, a feeling — anything."
        rows={3}
