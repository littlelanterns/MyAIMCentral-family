...
export function LilaModeSwitcher({ currentMode, modes, onModeSelect }: LilaModeSwitcherProps) {
  ...
  const currentLabel = DISPLAY_OVERRIDES[currentMode] || currentModeData?.display_name || 'LiLa Assist'
  ...
  const coreModes = drawerModes.filter(m => ['help', 'assist', 'optimizer'].includes(m.mode_key))
  const guidedModes = drawerModes.filter(m => !['help', 'assist', 'optimizer'].includes(m.mode_key))
