      ...
      >
        ...
        {view === 'month' && monthWeeks.map((week, wi) => (
          <div key={wi} className="grid grid-cols-7" style={{ borderBottom: wi < monthWeeks.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
            {week.map((date, di) => {
              if (!date) return <div key={`e-${wi}-${di}`} className="min-h-[72px]" style={{ borderRight: di < 6 ? '1px solid var(--color-border)' : 'none' }} />
              ...
              return (
                <button
                  ...
                  className="min-h-[72px] p-1 text-left flex flex-col"
                  style={{
                    background: isToday ? 'color-mix(in srgb, var(--color-btn-primary-bg) 8%, transparent)' : 'transparent',
                ...
                >
                  <span
                    className="text-xs font-medium self-end rounded-full flex items-center justify-center"
                    style={{
                      width: '24px',
                      height: '24px',
                    ...
                    {dayEvs.slice(0, 3).map(ev => (
                      <span
                        ...
                        style={{
                          ...
                          opacity: ev.status === 'pending_approval' ? 0.4 : 1,
                        }}
                      />
                    ))}
                    {dayTasks.slice(0, 2).map(t => (
                      <span key={t.id} className="rounded-sm" style={{ width: '6px', height: '6px', backgroundColor: 'var(--color-text-secondary)', opacity: 0.5 }} />
                    ))}
                    {(dayEvs.length + dayTasks.length) > 5 && (
                      <span className="text-[8px]" style={{ color: 'var(--color-text-secondary)' }}>+{dayEvs.length + dayTasks.length - 5}</span>
                    )}
                  </div>
                </button>
              )
            ...
            {weekDays.map((date, i) => {
              ...
              return (
                <button
                  key={key}
                  onClick={() => handleDateClick(date)}
                  className="min-h-[120px] p-2 flex flex-col"
                  style={{
                    background: isToday ? 'color-mix(in srgb, var(--color-btn-primary-bg) 8%, transparent)' : 'transparent',
                ...
                >
                  <span
                    className="text-sm font-semibold rounded-full flex items-center justify-center self-center mb-1"
                    style={{
                      width: '28px',
                      height: '28px',
                      color: isToday ? 'var(--color-btn-primary-text)' : 'var(--color-text-primary)',
