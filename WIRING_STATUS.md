...
Family-level goal system: the whole family works toward a shared prize together (practical core of the never-written PRD-24C, FD-7). Migration 100284 (`family_goals`, `family_goal_sources`, `family_goal_contributions`, `earned_prizes.family_member_id` nullable). Purpose-built DB-trigger contribution counting — NEVER routed through contracts/deed_firings/godmothers (Key Decision #2: `contracts.family_member_id IS NULL` already means "applies to each kid independently," an irreconcilable collision with "one shared goal"). E2E `tests/e2e/features/family-goals-prizes.spec.ts` 10/10 (incl. a concurrency probe proving exactly one prize is ever awarded at a threshold race) + eyes-on tour (`family-goals-prizes-eyes-on-tour.spec.ts`, Convention #277, all 3 viewports × mom/participant/non-participant roles, all screenshots read and verified) + zero fixture residue.

| Capability | How It Works | Status | Notes |
|---|---|---|---|
| `family_goals` / `family_goal_sources` / `family_goal_contributions` schema | 3 new tables; append-only contributions ledger (no client INSERT policy — writes only via SECURITY DEFINER trigger functions); `family_goal_sources` extensible `source_kind` (`family_intention` \| `task` today) | **Wired** | Migration 100284 |
| `earned_prizes.family_member_id` nullable — Family Prize = ownerless row | `DROP NOT NULL` + `CHECK (family_member_id IS NOT NULL OR source_type='family_goal')` + partial UNIQUE index on `(source_id) WHERE source_type='family_goal'` (double-award backstop). Pre-flight audit confirmed the 100266 `visibility='family'` RLS arm and `redeem_own_prize()` already resolve correctly for NULL — **zero RLS policy rewrite needed** | **Wired** | Migration 100284; verified via direct policy read, not assumed |
| Contribution counting — family intention taps | `count_family_goal_contribution_from_intention()` AFTER INSERT trigger on `family_intention_iterations`; window-checked at family timezone; idempotent via UNIQUE constraint | **Wired** | Fires regardless of client path (Hub tap, dashboard widget, family-device shadow session) |
| Contribution counting — task completions | `count_family_goal_contribution_from_completion()` AFTER INSERT OR UPDATE OF approval_status, completion_type on `task_completions`; predicate mirrors migration 100278's `award_custom_reward_for_completion` (the authoritative live Convention #200 filter, NOT the dropped `roll_creature_for_completion`); approval-required tasks count only at approval | **Wired** | Verified live: `task_completions_completion_type_check` only allows `('complete','practice','mastery_submit')` — trigger uses NOT-IN-exclusion framing so it's forward-compatible if `'mastery_approved'` is ever added |
| Award evaluation — race-safe, exactly-once | `evaluate_family_goal_award()` — status-guarded `UPDATE ... WHERE status='active'` (row lock serializes concurrent evaluators) + prize INSERT in the same transaction + `ON CONFLICT` against the partial unique index as backstop. `EXECUTE` revoked from PUBLIC/anon/authenticated — trigger-invocation only | **Wired** | E2E concurrency probe: 2 truly-concurrent threshold-crossing contributions → exactly 1 `earned_prizes` row, always |
| Mode A ("All together" / `shared_counter`) | One shared counter; any participant's contribution counts toward the family total | **Wired** | |
| Mode B ("Everyone does their part" / `each_member`) | Goal completes only when EVERY participant's own contribution count reaches the target | **Wired** | |
| `FamilyGoalManager` (two doors: Prize Board + Hub Settings) | Active/Completed/Archived groups; create/edit/archive; Duplicate-from-completed (Key Decision #16); full creation form (title, description, prize name/text/image via `RewardImagePicker`, participant `MemberPillSelector`, mode segmented control, target, Family Best Intention + task source pickers, optional end date, progress-visible toggle, Heart `is_included_in_ai` toggle) | **Wired** | `src/components/rewards/FamilyGoalManager.tsx` |
| Prize Board Prizes tab — Family group + Family Goals strip | By-kid arrangement gains a "Family" group (sorted first); By-date gets a "Family" tag; summary strip excludes family prizes from the "N kids" count, surfaces "+N family prize(s)" separately; `FamilyGoalsStrip` shows active goals w/ progress + prize preview + quiet `[+ Family goal]` affordance when none exist | **Wired** | `src/pages/PrizeBoard.tsx`, `src/components/rewards/FamilyGoalsStrip.tsx` |
| Hub `family_goals` section | New section key grafted via `mergeSectionOrder` (ported from `useFamilyOverviewConfig.ts`'s pattern — Hub had never had this helper before); Mode A progress bar / Mode B per-member mini-progress in member colors; ≤48h celebration banner "You did it! [Prize] earned!"; hidden when empty except mom's warm empty state; respects `progress_visible=false` | **Wired** | `src/components/hub/sections/HubFamilyGoalsSection.tsx`; `HUB_SECTION_KEYS` + 3 `SECTION_LABELS` maps updated |
| Hub Settings "Family Goals & Prizes" door | New settings group beside Family Best Intentions Management, opens the same `FamilyGoalManager` | **Wired** | `src/components/hub/HubSettings.tsx` |
| My Rewards Family section | Participant-only (FD-2, membership in `participating_member_ids`); "You: X · Family: Y/Target" (Mode A) / "You: X / Target" (Mode B) framing; earned unredeemed family prizes; redeemed family prizes merged into the shared `RedeemedHistoryModal` via a new optional `additionalRedeemed` prop; `my_rewards_sections.family` toggle (default ON, incl. Play) | **Wired** | `src/components/rewards/MyRewards.tsx`; verified non-participant sees the correct empty state, not the goal |
| Nullable-member consumer audit (Rider 1) | 6 readers audited — `useRewardProvenance` (safe, no assumption), `PrizesSection` (2 fixed: `viewableIds.has(null)` scoping gap + `grouped` keying), `RedeemedHistoryModal` (no assumption, extended with `additionalRedeemed`), `PlayRewards` (no direct assumption, inherits via `MyRewards`), `useRewardReveals`/`types/reward-reveals.ts` (types widened), `useEarnedPrizes.ts` (type widened + missing 100266 fields backfilled: `visibility`, `shared_with_member_ids`, `created_by`, `awarded_completion_id`) | **Wired** | Full findings table in `.claude/rules/current-builds/FAMILY-GOALS-PRIZES.md` |
| FeatureGuide + LiLa knowledge | `<FeatureGuide featureKey="family_goals" />` on the manager; `help-patterns.ts` pattern; `feature-guide-knowledge.ts` PAGE_KNOWLEDGE (`/prize-board`, `/my-rewards`) + new USE_CASE_RECIPE | **Wired** | |
| Boss Battle/Bingo visuals, "everyone completes everything on day X" goal type, money-payload prizes, LiLa suggestions + NLC/Studio wizard, FO section, PlayRewards display polish, reveal-ceremony animation, award notifications, realtime progress, contribution rewind, LiLa context assembly, repeating goals, victory/tracker/homework source kinds | — | Stub | Full rationale per stub in `STUB_REGISTRY.md` → "Family Goals & Family Prizes" |
...
| Queue "mine by default" for mom | SortTab owner filter pills (Mine / members-with-items / All), mom-only; default shows HER items; Deploy-all operates on the visible set; "Nothing waiting for you — N items in other queues" empty state. Mirrors the Tasks-page mine-by-default ruling. | **Wired** | Founder ruling same day (dad's queued brain-dump items were appearing in mom's queue — mom-sees-all RLS is by design; display now defaults to mine). E2E in review-route-direct-deploy.spec.ts. |
...
| FO Finances for finance-granted dads | Finances tab shows for mom OR finance-granted dad (`useManagementGrants`); display scoped to granted kids (per-kid wins incl. 'none' carve-out over family-wide); level-gated inside FinancesTab: view = read-only, contribute = + Pay All, manage = + grace days/makeup | **Wired** | Founder add-on 2026-06-10; Convention #274 levels; E2E test 8b |
| FO per-column long-press collapse override + section/column drag reorder (PRD-14C) | Tap section header = row-level collapse (all columns); HOLD (500ms, 8px slop) = just that column (override dot, cleared by row toggle); ⠿ grips reorder sections globally + columns horizontally via @dnd-kit; both persist to `family_overview_configs` | **Wired** | Founder add-on 2026-06-10; closes the pre-existing PRD-14C gap; selectedMembers memo fixed (newly selected members append after a saved column_order instead of vanishing); E2E test 8c |
| FO compact member pill selector | `MemberPillSelector` gains `variant="compact"` — small content-sized pills in one wrapping row (the app-standard pill bar); FO uses it; grid variant stays default for creation flows (PRD-14B Pick Members untouched) | **Wired** | Founder eyes-on feedback 2026-06-10 ("bulky and inconsistent") |
| FO tap-to-edit on every column item | Tasks/Routines/Opportunities rows → full `TaskEditModal` (`openEditTaskById` fetches the full row); Sequential rows → shared `SequentialDetailModal` (extracted from Tasks.tsx); Trackers → `WidgetDetailView` (with data recording); Events → `EventCreationModal` edit mode. Handlers wire only for members the viewer can act on (Decision 9 contribute+); view-only rows stay display-only | **Wired** | Founder eyes-on feedback 2026-06-10 ("edit it from right there"). Best Intentions/Victories rows remain display-only (their editors live on Guiding Stars/Victories pages — flag if wanted) |
| FO segment grouping for segmented kids (Build M) | When a member has active `task_segments` (Play/Guided especially), their column's Tasks section groups items under segment headers (per-segment done/total, day-filter aware via `isSegmentActiveToday`); tapping a header opens that kid's `GamificationSettingsModal` (Day Segments CRUD — rename, icons, days, reorder). Mom-only header tap; flat list preserved for members without segments (Convention #208) | **Wired** | Founder ask 2026-06-10. Picture-picking on Play tasks was ALREADY live via tap-to-edit (TaskCreationModal's built-in `TaskIconPicker` shows for Play assignees) |
| FO per-section [+ create] buttons | Mom-only Plus on each creatable section header opens the category's creation modal pre-targeted at that member: tasks/routines/opportunities → TaskCreationModal (new additive `initialAssigneeId` prop, checkbox-honesty guarded); sequential → SequentialCreatorModal; events → EventCreationModal (new additive `initialAttendeeIds`); best_intentions → new `CreateBestIntentionModal`; victories → new `RecordVictoryModal` (description + importance chips, source='manual'); trackers → WidgetPicker → WidgetConfiguration (deploys to the column's member). Only Weekly Completion has no + (allowance config) | **Wired** | Founder asks 2026-06-10 ("another entrance point... all on one surface" + "record those things as victories"); additive modal props noted in the cross-session coordination file |
| FO claimed opportunities + mom unclaim | Opportunities section now shows ACTIVE claims (both shapes: list-board claim bridge tasks with status pending/in_progress/pending_approval AND `task_claims` rows on standalone claimables) above today's completions, with a mom-only [Return] button. Unclaim: list claims release any task_claims rows then cancel+archive the bridge task (the availability check frees the item back to its board); standalone claims release the task_claims row (useReleaseClaim semantics). `useUnclaimOpportunity` lives in `useOpportunityLists.ts` (claim-bridge domain home) | **Wired** | Founder eyes-on finding 2026-06-10 (Gideon's claimed items showed "(none today)") |
| Kid voluntary claim release ("Put it back") | PRD-09A's never-built release, now on THREE kid surfaces: (1) opportunity board card (`OpportunityListBrowse`) when claimed by you and not pending approval; (2) standalone opportunity dashboard card (`OpportunityDashboardView`) via `useReleaseClaim`; (3) the claim bridge task on the kid's own task list (`TaskCard` — `source='opportunity_list_claim'`, own assignee, Undo2 icon). No penalty; item returns to the board for anyone | **Wired** | Founder ask 2026-06-10 ("yes please add that ability for the kids to release the claim themselves") |
...
| My Rewards page + `show_my_rewards` toggle | **STALE ROW UPDATED (KIDS-REWARDS-PAGE, 2026-06 through 2026-07):** `/my-rewards` is no longer a stub — the `PlannedExpansionCard` was replaced by the real `<MyRewards>` component (Slices 1-4): Points/streak, Custom Rewards (redeem + history), Victories, Finances, Creatures, Coloring, and Propose-a-Reward/self-propose sections, each mom-toggleable per member via Gamification Settings. Sidebar/nav entry still gated by per-member `family_members.preferences.show_my_rewards` (now defaults ON — only an explicit false hides it). See `.claude/completed-builds/2026-07/kids-rewards-page.md` for the full build record. | **Wired** | Follow-Up Build A (referenced below) is COMPLETE — this was the build. |
