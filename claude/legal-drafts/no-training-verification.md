...
### 2.1 Code hardening — **IMPLEMENTED AND DEPLOYED 2026-07-05 (NOTRAIN-HARDEN)**

Founder approved option (b) below on 2026-07-05. `supabase/functions/_shared/openrouter-client.ts`
now exists, every call site identified in §2 was swept onto it, and all 36 affected Edge Functions
were redeployed to production the same day — see the updated §7 condition 3 row for the full detail
and regression-test reference. This section is kept as the historical record of the two options
...
| # | Condition | Met now? |
|---|---|---|
| 1 | OpenRouter account dashboard confirmed to block routing to any training/logging provider (paid **and** free model settings), and OpenRouter's own product-improvement/Observability toggles confirmed off | ✅ **MET 2026-07-05** — founder verified live and corrected one setting: "Paid endpoints that may train" OFF (was already), "Free endpoints that may train" OFF (**was ON; turned OFF by founder 2026-07-05** — note: the app pins paid Anthropic models, so free-endpoint routing was almost certainly never exercised, but the exception is now closed), "Free endpoints that may publish prompts" OFF, "Allow 1% data discount" OFF. ZDR restriction toggles deliberately left OFF (stricter than required; would disable first-party Anthropic endpoints). **Exhibit:** `exhibit-openrouter-data-policies-2026-07-05.png` (verified by coordination-seat screenshot read same day). |
| 2 | OpenAI organization Data Controls confirmed **Disabled** for both toggles | ✅ **MET 2026-07-05** — founder verified live: Sharing tab shows ALL THREE sharing settings **Disabled** (model-feedback sharing, evaluation/fine-tuning data sharing, share-inputs-and-outputs). **Exhibit:** `exhibit-openai-sharing-disabled-2026-07-05.png`. Related but non-blocking: the Data-retention tab's org-internal "API call logging" was **"Enabled per call"** at capture (`exhibit-openai-api-logging-2026-07-05.png`) — this is org-internal review logging, NOT sharing/training; the app never opts a call in; recommended set to Disabled for cleanliness but it does not gate this claim. |
| 3 | Code-level `provider: { data_collection: 'deny' }` hardening implemented across all OpenRouter call sites (or a new shared helper), so the claim is enforced structurally rather than resting on account defaults that could silently change | ✅ **MET 2026-07-05 (NOTRAIN-HARDEN).** `supabase/functions/_shared/openrouter-client.ts` is now the single entry point for every OpenRouter request — `withNoTraining()` forces `provider: { data_collection: 'deny' }` onto every request body. All 48 call sites across all 36 Edge Functions that call OpenRouter were swept onto it (`callOpenRouter()` for the ~44 direct-fetch sites; `OPENROUTER_URL` + `openRouterHeaders()` + `withNoTraining()` for the 2 files — bookshelf-extract, bookshelf-study-guide — that wrap the call in their own generic retry helper). Founder approved deploying all 36 functions to production (project `vjfbzpliqialqmabfnxs`) same day; deploy succeeded. Regression-guarded by `tests/e2e/features/notrain-harden.spec.ts` — 45/45 passing: 41 static pins (every call-site file's import pinned individually, plus directory-wide greps asserting no file outside the shared client hardcodes the OpenRouter URL or sets `data_collection` directly) + 4 LIVE probes against the deployed functions confirming `provider.data_collection: "deny"` does not break OpenRouter's routing across 4 architecturally distinct request shapes: `ai-parse` (utility JSON), `task-breaker` (quick-subtask JSON), `mindsweep-sort` (batch classify JSON), and `lila-chat` (SSE streaming conversation). Zero test-fixture residue verified post-run. `npx tsc -b` and `npm run lint` both clean; SAFETY-BETA-GATE's 39 static pins re-verified green (no cross-contamination). This is now true "by design," not just "by settings." |
| 4 | Written confirmation obtained (from OpenRouter support, or from counsel's review of OpenRouter's actual Model Provider Agreement with Anthropic) that Anthropic's no-training commitment is the operative term for data routed through OpenRouter to us — closing the privity gap identified in §3–4 | **Not done** — this is the item only OpenRouter or counsel can close; founder to send the §6 support-ticket question; flagged as part of Q3 in `data-practices-summary.md` |
...
**Bottom line (updated 2026-07-05, end of day): conditions 1, 2, and 3 are all now MET** —
1 and 2 with dashboard exhibits, 3 deployed to production and live-verified via
`tests/e2e/features/notrain-harden.spec.ts` (45/45 passing). **Only 4 (the OpenRouter privity
question) and 5 (counsel sign-off) remain.** The sentence stays bracketed until 4 and 5 close.
Item 3 has now shipped the claim from "true today, by settings" to "true by design," which
matters given how much weight a COPPA disclosure
puts on this exact sentence.
