...
| Mom-UI verification table fully ✅ | Wired (Desktop; Tablet/Mobile waived for cause) | Founder eyes-on deployed myaimcentral.com 2026-06-08 — confirmed phantom painted routines no longer appear on the weeks they previously polluted (Gideon/Miriam/Mosiah history); unaffected weeks render normally. Tablet/Mobile `⊘ waived` — data-only change, no layout modification (founder: "more of a data issue, not physical or visual"). |
| Founder-visible allowance amounts reviewed; any surprises documented | Reviewed qualitatively; per-kid delta capture deferred | Founder confirmed phantom jobs gone (denominators corrected); exact old→corrected numbers not yet captured. Open follow-up: founder reviews per-kid deltas (Gideon/Miriam/Mosiah) during this-week testing and decides honor-old-vs-corrected per kid. **Default: corrected.** Smoke evidence: Gideon week 5/24–5/30 = 52.78% with expired Upper Common Room excluded. |
