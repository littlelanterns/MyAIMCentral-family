...
| PRD-28-Phase-3.5-Multi-Pool-Allowance.md | PRD-28 | 2026-05-06 | Complete — Multi-Pool Allowance. 14 capabilities, 3 fold-in items, 18 key decisions. Migrations 100234-100236. 17 Playwright tests. 4 stubs. |
| View-As-Identity-Scope-Architecture.md | PRD-02 + PRD-14D + PRD-28 | 2026-05 | Workers 1–6 code complete; **pending Checkpoint 5 founder eyes-on + Checkpoint 6 close-out**. Modal-overlay identity scope (`useEffectiveMember/Shell/Viewer`), `view_as_sessions.origin` column (migration 100246), `<MomOnlyRoute>` backstop (16 routes), `filterKidPrivate()`, hub PIN modal-over-`/hub`, modal-only timeout, My Rewards stub (migration 100248). Conventions #39 (rewritten) + #272 (per-instance Realtime). 7 follow-up builds (A–G). 2 Playwright smoke tests. |
