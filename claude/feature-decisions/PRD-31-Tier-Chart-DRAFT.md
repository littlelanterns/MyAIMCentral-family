...
> **Status: ENCODED v2 — LIVING DRAFT (Slice 1 applied to production 2026-08-23, migration 100316).** The approved principles are now LIVE in the seeds: (A) every ENABLED non-mom `E` cell was mechanically converted to `En` (151 rows); (D) `safety_monitoring_basic` mom cell moved to Essential, AI layer stays FM; all 45 `?` NULL-tier rows repaired per the inline suggestions; §N hygiene executed (merges, retirements, registrations, display-name fixes, `vault_hearts`/`vault_comments_*` added — the comments non-mom cells are the platform's first explicit Never rows). **The founder's cell-level markup still happens on her schedule — this chart remains a living draft until flip time; later marks flow in via seed updates or the Screen-4 admin grid once built.** Cell values below reflect the PRE-encoding 2026-07-08 snapshot — read them through the A-principle (non-mom E → En) until Screen 4 supersedes this file; the live seeds are ground truth, pinned by `tests/prd31-registry-completeness.test.ts`.
>
> **Row semantics (R31-2, now enforced in production):** a `feature_access_v2` row with `is_enabled=false` = **Never** — not available to that role group at any tier (`minimum_tier_id` is retained but inert on disabled rows; Screen 4's ✕ cell writes `is_enabled=false`, never deletes). An **absent row** = not-yet-assigned, and the activation default for absent rows is **UNGATED (allowed)** — nothing silently disappears at switch-flip because a seed row was missing (Rule 1). Founder exceptions to the A-principle get encoded in the vitest's `FOUNDER_EXCEPTIONS` allowlist alongside the seed update.
