...
- [ ] **Server-side auto-extract queue** — opt-in checkbox in BookUploadModal (default OFF): `[ ] Auto-extract after upload`. When checked, `bookshelf-process` sets `extraction_status = 'queued'` after chunking. New pg_cron job (every 60s) picks up one `'queued'` book, calls a new `bookshelf-extract-batch` Edge Function that runs discover_sections → combined_section per section server-side. Progress shown via `bookshelf_items.processing_detail`. Multiple uploads queue FIFO, one book at a time. Costs ~$0.50-1.50/book in Sonnet. Founder decided 2026-04-10: default OFF (no surprise AI costs), user opts in per upload session. Scope: 1b-E or 1b-F, or post-Phase-1b polish — wherever it fits after the read-path rewires are stable.
- [ ] **Frontend surface for guided_text / independent_text** — the platform `book_extractions` table has `guided_text` (ages 8-12) and `independent_text` (ages 13-16) populated on every new extraction since 1b-B, but no UI renders them yet. Three natural surfaces: (a) View As Guided/Independent member auto-switches to youth text, (b) Guided/Independent shell BookShelf auto-shows age-appropriate version, (c) Study Guide page reads from these columns instead of re-generating via Haiku. Key off `family_members.dashboard_mode`.
