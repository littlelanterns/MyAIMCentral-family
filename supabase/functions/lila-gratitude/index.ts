...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ethicsReframe = await handleEthicsInputReframe(supabase, content, {
      familyId: conv.family_id, memberId: conv.member_id, surface: 'lila-gratitude', modeKey: 'gratitude',
    ...
    return createSSEStream(async (enqueue) => {
      ...
      const { data: savedMsg } = await supabase.from('lila_messages').insert({ conversation_id, role: 'assistant', content: fullText, metadata: { model: MODEL, mode: 'gratitude', ...(ethicsScan.retractionMetadata ?? {}) }, token_count: outputTokens }).select('id').single()
