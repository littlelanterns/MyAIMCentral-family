...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ethicsReframe = await handleEthicsInputReframe(supabase, content, {
      familyId: conv.family_id, memberId: conv.member_id, surface: 'lila-words-affirmation', modeKey: 'words_affirmation',
    ...
    const aiRes = await callOpenRouter(
      ...
      { title: 'MyAIM Central - Words Affirmation' },
    )
    ...
    return createSSEStream(async (enqueue) => {
      ...
      const { data: savedMsg } = await supabase.from('lila_messages').insert({ conversation_id, role: 'assistant', content: fullText, metadata: { model: MODEL, mode: 'words_affirmation', ...(ethicsScan.retractionMetadata ?? {}) }, token_count: outputTokens }).select('id').single()
