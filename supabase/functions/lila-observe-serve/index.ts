...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ethicsReframe = await handleEthicsInputReframe(supabase, content, {
      familyId: conv.family_id, memberId: conv.member_id, surface: 'lila-observe-serve', modeKey: 'observe_serve',
    ...
    const ctx = await loadRelationshipContext(conv.family_id, conv.member_id, personIds, 'observe_serve', content, recentMsgs)
    ...
    const aiRes = await callOpenRouter(
      ...
      { title: 'MyAIM Central - Observe Serve' },
    )
    ...
    return createSSEStream(async (enqueue) => {
      ...
      const { data: savedMsg } = await supabase.from('lila_messages').insert({ conversation_id, role: 'assistant', content: fullText, metadata: { model: MODEL, mode: 'observe_serve', ...(ethicsScan.retractionMetadata ?? {}) }, token_count: outputTokens }).select('id').single()
