-- ============================================================================
-- 100243 — Archived tasks must not silently penalize allowance
--
-- Bug: When a routine is archived mid-period (via "Remove from dashboard" or
-- "Delete"), the task's `archived_at` is set. The allowance RPC's task loop
-- filter `AND t.archived_at IS NULL` then drops the task from BOTH the
-- denominator and numerator. But completions already recorded against it
-- earlier in the period become orphaned — they contributed to the numerator
-- via routine_step_completions but the task that anchored them in the
-- denominator is gone. The kid's percentage can swing unpredictably.
--
-- Correct behavior: if a task was archived during the active period, it
-- should still count in the calculation for the portion of the period it
-- was active. Tasks archived before the period starts are fully excluded.
--
-- Fix: change `AND t.archived_at IS NULL` to
--   `AND (t.archived_at IS NULL OR t.archived_at::DATE > p_period_start)`
-- This means:
--   - Active tasks (archived_at IS NULL): counted as before
--   - Tasks archived DURING the period: still counted (completions before
--     archive date are real work the kid did)
--   - Tasks archived BEFORE the period started: excluded (they were already
--     hidden when the period began)
--
-- The denominator for mid-period-archived routines is naturally limited
-- because the routine's steps only generate denominator days up to
-- p_period_end. The kid can't complete steps after archival, so the
-- numerator naturally stops growing — but the work they DID do stays
-- credited.
-- ============================================================================
...
BEGIN
  ...
  -- ── FIX (100243): include tasks archived DURING the period ──
  -- Old filter: AND t.archived_at IS NULL
  -- New filter: active OR archived after the period started
  -- Tasks archived before the period are fully excluded.
  FOR v_task IN
  ...
  LOOP
    ...
    -- For archived tasks, cap the effective end at the archive date
    -- so the denominator only counts days the task was actually visible
    ...
    IF v_task.task_type = 'routine' AND v_task.template_id IS NOT NULL THEN
      ...
      -- Use v_effective_end instead of p_period_end for archived routines
      ...
      -- Also capped at v_effective_end for archived routines
      SELECT COUNT(*)::INT
...
COMMENT ON FUNCTION public.calculate_allowance_progress(UUID, DATE, DATE, JSONB, TEXT) IS
  '100243: archived tasks no longer silently penalize allowance. Tasks '
  'archived during the period count proportionally (denominator and numerator '
  'capped at the archive date). Tasks archived before period start are excluded. '
  'Preserves: show_until_complete (100239), orphan counting (100241), '
  'multi-pool (100235), numerator boost, grace days.';

DO $$ BEGIN
  RAISE NOTICE 'migration 100243: archived-task allowance fix applied';
  RAISE NOTICE '  Tasks archived mid-period now count proportionally';
  RAISE NOTICE '  Denominator and numerator capped at archive date';
END $$;
