...
-- wizard_drafts (subject: member_id)
DO $do$
BEGIN
  IF to_regclass('public.wizard_drafts') IS NULL THEN
    RAISE NOTICE 'coppa write gates: public.wizard_drafts absent — skipped';
  ELSE
    EXECUTE 'DROP POLICY IF EXISTS coppa_write_gate_ins ON public.wizard_drafts';
    EXECUTE 'CREATE POLICY coppa_write_gate_ins ON public.wizard_drafts AS RESTRICTIVE FOR INSERT WITH CHECK (util.coppa_write_allowed(member_id))';
    EXECUTE 'DROP POLICY IF EXISTS coppa_write_gate_upd ON public.wizard_drafts';
    EXECUTE 'CREATE POLICY coppa_write_gate_upd ON public.wizard_drafts AS RESTRICTIVE FOR UPDATE USING (true) WITH CHECK (util.coppa_write_allowed(member_id))';
  END IF;
END $do$;
