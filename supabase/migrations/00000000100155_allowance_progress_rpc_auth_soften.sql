-- Soften the authorization check in calculate_allowance_progress so it works
-- for all family member roles, not just authenticated adults with a user_id.
-- Play/Guided children don't always have a user_id populated (PIN-only auth),
-- and session contexts for various View-As flows may carry different claims.
-- The RLS policies on allowance_configs, tasks, and routine_step_completions
-- already restrict data to family members, so this RPC can trust the union of
-- (a) caller is service_role, OR
-- (b) caller has a family_members row in the same family as the target, OR
-- (c) caller IS the target member (their own auth.uid() === family_members.user_id).
-- If auth.uid() is NULL (unauthenticated / anon), reject.
...
BEGIN
  ...
  ELSIF auth.uid() IS NOT NULL THEN
    -- Same family as the target member, via either user_id match or
    -- same-family membership check.
    SELECT EXISTS (
