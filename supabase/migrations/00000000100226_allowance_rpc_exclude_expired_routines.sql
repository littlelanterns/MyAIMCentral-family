-- Fix: Exclude expired routines from allowance calculation.
--
-- BUG: calculate_allowance_progress iterates ALL routines where
-- counts_for_allowance=true AND archived_at IS NULL. But routines with
-- recurrence_details->>'until' before the period start are expired — the
-- frontend hides them via recurringTaskFilter.ts, but the RPC still counts
-- their steps in the denominator (inflating "0/N steps" when the routine
-- is invisible). Same issue in check_day_completion.
--
-- FIX: Add a WHERE clause that skips routines whose until date is before
-- the relevant boundary (p_period_start for allowance, CURRENT_DATE for
-- day-completion).
--
-- Also adds a client-callable archive_expired_routines RPC for the
-- frontend sweep hook.
--
-- Preserves all prior corrections: 100164 frequency-day-aware, 100171
-- extra credit, 100172 grace days, 100175 per-day grace mode, 100190
-- actual-completer. Signature unchanged (UUID, DATE, DATE, JSONB).

-- ═══════════════════════════════════════════════════════════════════════
-- 1. calculate_allowance_progress — expired routine filter
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- V8 FIX (100190): Widen cursor via task_assignments. DISTINCT on t.id.
  -- V9 FIX (100226): Exclude expired routines — routines whose
  -- recurrence_details->>'until' is before the period start are done and
  ...
  LOOP
    ...
    -- Extra-credit branch (migration 100171)
    ...
    IF v_task.task_type = 'routine' AND v_task.template_id IS NOT NULL THEN
      -- Routine branch: day-by-day step count with frequency filtering.
      -- V9 FIX: Also bound the day walk to the routine's until date so
      -- steps past the end date are not counted in the denominator.
      v_routine_total_possible := 0;
      v_day := v_effective_start;
      -- V9: bound the day walk to the routine's until date so steps past
      -- the end date are not counted in the denominator.
      v_routine_end := LEAST(
        p_period_end,
        COALESCE(
...
COMMENT ON FUNCTION public.calculate_allowance_progress(UUID, DATE, DATE, JSONB) IS
  'V9 expired-routine fix (migration 100226). Routines with recurrence_details.until before the period start are excluded from the task cursor (they were inflating the denominator with ghost steps). The day-walk loop is also bounded by the until date so mid-period expirations only count steps up to the end date. Preserves all prior corrections: 100164, 100171, 100172, 100175, 100190.';


-- ═══════════════════════════════════════════════════════════════════════
-- 2. check_day_completion — expired routine filter
-- ═══════════════════════════════════════════════════════════════════════
  ...
  SELECT count(DISTINCT t.id) INTO v_incomplete
   ...
   WHERE (t.assignee_id = p_member_id OR ta.member_id IS NOT NULL)
     ...
     AND (
       ...
       OR t.recurrence_details IS NULL
       OR (t.recurrence_details->>'until') IS NULL
       OR (t.recurrence_details->>'until')::DATE >= CURRENT_DATE
     )
     AND NOT EXISTS (
...
COMMENT ON FUNCTION public.check_day_completion(UUID) IS
  'V9 expired-routine fix (migration 100226). Excludes routines past their recurrence_details.until date from incomplete-task counting.';


-- ═══════════════════════════════════════════════════════════════════════
-- 3. archive_expired_routines — client-callable sweep RPC
-- ═══════════════════════════════════════════════════════════════════════
--
-- Called by the frontend on Tasks/Dashboard load. Archives routines whose
-- until date has passed. Returns the count of archived rows so the caller
-- can log or toast.
...
GRANT EXECUTE ON FUNCTION public.archive_expired_routines(UUID)
  TO authenticated, service_role;

COMMENT ON FUNCTION public.archive_expired_routines(UUID) IS
  'Sweep: archives all routines in a family whose recurrence_details.until has passed. Called by the frontend on page load as a belt-and-suspenders cleanup. Returns count of archived rows.';


-- Verification
DO $$ BEGIN
  RAISE NOTICE 'migration 100226: expired-routine filter applied';
  RAISE NOTICE '  calculate_allowance_progress: V9 with until-date filter';
  RAISE NOTICE '  check_day_completion: V9 with until-date filter';
  RAISE NOTICE '  archive_expired_routines: new sweep RPC';
END $$;
