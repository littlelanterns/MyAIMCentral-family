-- Extend calculate_allowance_progress with raw step counts so the widget can
-- display meaningful "X / Y steps this week" rather than a rounded pro-rated
-- task fraction that collapses to 0/2 for any mid-week state. Also returns
-- non-routine task completion counts for mixed pools.
--
-- Return signature changes — must DROP before CREATE.
...
BEGIN
  ...
  LOOP
    ...
    IF v_task.task_type = 'routine' AND v_task.template_id IS NOT NULL THEN
      ...
      v_raw_steps_completed := v_raw_steps_completed + LEAST(v_routine_actual_completed, v_routine_total_possible);
