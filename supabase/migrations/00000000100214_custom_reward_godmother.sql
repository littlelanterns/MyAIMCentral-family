-- Phase 3 Connector Architecture — Sub-task 14
-- custom_reward_godmother: delivers custom rewards via two modes:
--   'text'           — IOU with mom-defined text
--   'list_reference' — picks next/random item from a rewards list
-- Writes to earned_prizes for display on mom's Prize Board.
...
BEGIN
  ...
  IF v_delivery_mode = 'list_reference' THEN
    ...
    -- Pick a random available item from the list
    ...
    IF v_list_item.id IS NULL THEN
      -- No available items left; fall back to text mode with a generic message
    ...
    ELSE
      ...
      -- Mark item as checked (awarded) if it's one-time
      ...
      -- For repeatable items, increment completion count
      UPDATE public.list_items
    ...
    END IF;
  ELSE
    -- Text mode: use config text or inline payload
