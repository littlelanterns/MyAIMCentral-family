...
275. **Family Overview is mom's command center; the Tasks page is purely personal.** Founder-approved 2026-06-10 (vision verbatim in `claude/feature-decisions/Family-Overview-Command-Center-Vision.md`; supersedes PRD-09A Screen 1's Tasks-page management-tab model and the Studio Intelligence addendum's "Sequential tab dual access, not moved" ruling — newer wins per Convention #11). The rules:
    ...
    - **Family Overview page tabs:** Overview / Approvals(N) / Queue(N) / Finances. Queue (mounts the real `SortTab` — the modal Sort surface; Conventions #66/#146 modal contract untouched) and Finances (relocated `FinancesTab`, founder Q7) are mom-only; Approvals is scoped per viewer via `filterVisiblePendingApprovals`. The UniversalQueueModal and all its entry points (QuickTasks strip, QueueBadge, etc.) are unchanged.
