...
273. **Every family is one mom account with two login doors; member access always happens INSIDE an umbrella.** Founder-approved 2026-06-09; supersedes PRD-01 Screen 4's public roster flow. The rules:
    ...
    - **Known gap (STUB_REGISTRY):** personal-device member timeout still fully signs out (kid re-enters the family password). The intended two-layer stickiness (family layer persists; timeout drops only to the PIN/picture relock) is a committed follow-up.
    - E2E pin: `tests/e2e/features/family-auth-two-door.spec.ts` (8 tests) must stay green — it guards the no-enumeration property, roster gating, mom-tile gating, and the kill-switch pipeline.
