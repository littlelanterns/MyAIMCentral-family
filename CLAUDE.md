...
274. **Every permission key the Hub offers is either enforced or visibly inactive; grantable management surfaces supersede "mom-only."** Founder-gated 2026-06-09 (amends the Phase 3 Connector Layer "Prize Board is mom-only" rule and PRD-02's never-wired Hub rows). The rules:
    ...
    - **Grant shapes:** `financial_tracking` is PER-KID (`member_permissions` target = kid). `studio` and `reward_rules` are FAMILY-WIDE: `target_member_id IS NULL` (column is nullable since migration 100260; partial unique index `uq_mp_family_wide` enforces one row per family+grantee+key).
