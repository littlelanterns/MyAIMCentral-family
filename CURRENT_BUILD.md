...
- `/sweep` route not in App.tsx yet
- No mindsweep tables exist in live database (need migration)
- No `mindsweep-sort` Edge Function exists
- No `/sweep` page component exists
- No `classify_by_embedding()` RPC exists
...
1. `/sweep` route in App.tsx
2. `MindSweepCapture.tsx` page component — text input (autofocus), voice button, scan button (ScanLine), link button
3. Voice optimization: Web Speech API for <30s, Whisper for 30s+ (modify useVoiceInput or create wrapper)
4. Scan: vision model OCR (extract from bookshelf-process pattern) — image NOT stored, text extraction only
5. Link capture: URL fetch + summarize
6. Holding queue UI: item count, next auto-sweep time, [Sweep Now], [Save for Later]
7. Auto-sweep: pg_cron job at configured time (default 8pm)
8. Share-to-app via Web Share Target API (Android; graceful iOS fallback)
9. PWA manifest for `/sweep` (distinct icon)
10. Basic IndexedDB offline capture
11. QuickTasks MindSweep button opens quick-capture
12. TypeScript check
