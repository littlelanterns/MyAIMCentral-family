...
## PRD-40 COPPA Compliance — Slice 6 (Admin + Close-Out) (2026-08-24)

Slice 6 shipped (CODE COMPLETE, migration 100330 written NOT applied — the batched seat ask is in the build file): `/admin/coppa` (Screen 10) as the fourth ADMIN_TABS row with four `coppa_admin`-gated SECURITY DEFINER RPCs, **the lawyer_approved_at STAMP GUARD** (the sequencing law encoded in `admin_stamp_consent_template` + the direct-write side door closed via column-level grants — a coppa_admin staff session can no longer stamp a template by bare `.update()`), LiLa knowledge (help pattern, page knowledge, use-case recipe, 2 FeatureGuide registry entries + mounts), the E2E coverage close (`coppa-admin-console.spec.ts` 7 tests incl. the Slice-2 reconciliation gap + `coppa-admin-eyes-on-tour.spec.ts` 4 tests + `scripts/coppa-admin-stamp-probes.sql` BEGIN…ROLLBACK success-path proof), and the founder backfill ceremony script (`claude/orchestration/PRD-40-Backfill-Ceremony-Script.md` — seat-run, founder-present, NOT run).

| Stub | Created By | Wired By | Status | Build Phase |
|------|-----------|----------|--------|-------------|
| Slice-6 production sequence (migration 100330 apply, stamp probes, 11-test suite run, rls-verifier pass, Conv-#277 tour) | PRD-40 Slice 6, 2026-08-24 | Seat-gated batched ask (build file) | ⏳ Unwired (MVP) | Everything is authored and parse/lint/type-verified locally; nothing production-touching has run. |
| Founder backfill ceremony (R-9) | PRD-40 Slice 6 | Founder + seat, from the script | ⏳ Unwired (MVP) | Scripted end-to-end incl. keepsake-exports-before-retention-crons ordering and the v1.0.0-referenced interim basis (flagged deviation from R-9's literal 0.9.0 seed). Until it runs, the stamp guard refuses all template approvals — by design. |
| Template-version authoring UI (typing new disclosure text in the admin tab) | PRD-40 Slice 6 | Not planned | 📌 Unwired (Post-MVP) | Deliberate: disclosure text is legal copy — new versions land as migrations/seat operations. The admin tab manages versions (stamp/retire/notes), never authors them. |
| FeatureGuide cards render nothing (platform-wide `FEATURE_GUIDES_DISABLED=true`) | Pre-existing (Stage 3, 2026-05-01) | Whenever the platform flag flips | 📌 Unwired (Post-MVP) | Slice 6 registered `coppa_consent_review` + `coppa_admin_log` content and mounted both — they light up with the flag, no COPPA work needed. |

---

## PRD-40 COPPA Compliance — Slice 5 (Enforcement) (2026-08-24)

Slice 5 shipped (CODE COMPLETE, migrations 100327-100329 written but NOT applied — every apply/deploy/suite-run is founder-gated per the production-touch gate): `util.coppa_write_allowed()` (migration 100327, R-8-dormancy-aware, fail-open-loudly), 268 GENERATED RESTRICTIVE write-gate policies across all 134 hard_delete registry tables (migration 100328, generator `scripts/coppa-write-gates.ts`, drift pin `tests/coppa-write-gates-consistency.test.ts`, rollback `scripts/coppa-write-gates-rollback.sql`), roster suspension filters (both roster RPCs + 8 frontend roster sites + both context assemblers), `useCoppaConsent()` + `useCoppaEnforcementActive()` + `coppaWriteBlocked()`, `_shared/coppa-consent.ts` (Edge twin of the SQL predicate), AI-call gates in `lila-chat` + `bookshelf-discuss`, the under-13 persona-promotion exclusion in `lila-board-of-directors`, `util.coppa_reconcile_age_brackets()` + daily cron (migration 100329 — closes the Slice-4 "age-transition job" stub), computeIsUnder13 bracket-first (R-2), and the aggregation-exclusion CI seed (`scripts/check-under13-aggregation.cjs`, wired into `prebuild`). See `.claude/rules/current-builds/PRD-40-coppa.md` Slice-5 progress log.

| Stub | Created By | Wired By | Status | Build Phase |
|------|-----------|----------|--------|-------------|
| SECURITY DEFINER write-RPC coverage (the gates' known boundary) | PRD-40 Slice 5, 2026-08-24 | Cohort-2 sweep (pre-activation) | 📌 Unwired (Post-MVP) | RESTRICTIVE RLS policies do not bind SECURITY DEFINER functions (they run as the table owner) or service-role Edge Functions. The Edge layer is covered for the conversation engines (`_shared/coppa-consent.ts` gates in lila-chat/bookshelf-discuss); the SQL RPC layer (e.g. `record_point_transaction`, `process_routine_step_completion`, `purchase_reward_shop_item`) is NOT gated — practically bounded today because every such RPC fires downstream of a client action that IS RLS-gated, and enforcement is dormant anyway (R-8). Before cohort-2 activation, sweep the SECURITY DEFINER write RPCs and add `util.coppa_write_allowed()` checks where a bare-RPC path could bypass the gates. |
| Per-surface "This child's profile is not yet set up" held-state message wiring | PRD-40 Slice 5 (PRD L548) | Enforcement activation (cohort-2 gate) | 📌 Unwired (Post-MVP) | `COPPA_HELD_PROFILE_MESSAGE` + `coppaWriteBlocked(state, enforcementActive)` + `useCoppaConsent()` are exported and ready; no surface wires them yet because no surface can REACH a blocked state today (suspended members are hidden from every roster; the unconsented-under-13 rule is dormant). Wire at the write-error surfaces when the founder flips enforcement on. |
| AI-gate utility-function sweep (beyond lila-chat + bookshelf-discuss) | PRD-40 Slice 5 | Cohort-2 sweep (pre-activation) | 📌 Unwired (Post-MVP) | `checkCoppaWriteAllowed()` is the standard; the two member-conversation ENGINES are gated (they persist member AI conversations and cover all 43+ LiLa guided modes via lila-chat). Remaining member-invocable utility AI functions (mindsweep-sort/-scan, task-breaker, message-coach, lila-translator, curriculum-parse, wishlist-extract, recipe-extract, …) each need the same two-line gate after member resolution. `tests/coppa-ai-gate.test.ts` pins the current minimum set. |
| Enforcement E2E full run + rls-verifier pass + tour | PRD-40 Slice 5 | **RUN 2026-08-24 (Slice-5 proof)** | ✅ Wired | Full nine-step sequence green: INERTNESS 4/4 identical pre/post-apply, ENFORCEMENT 2/2, AI GATE 1/1, probes rolled-back-clean, rls-verifier 26/26, tour 2/2 read, regressions green, zero residue. Migrations 100327-100329 applied; the three deploys done. |

---

## PRD-40 COPPA Compliance — Slice 4 (Rights + Lifecycle) (2026-08-24)

Slice 4 shipped: Screen 8 (Privacy & Consent), Screen 9 (Revocation Flow, 3 steps), `revoke_coppa_consent`/`undo_coppa_revocation` RPCs (migration 100322), per-child data export (`coppa-export-child-data` Edge Function, hand-rolled dependency-free ZIP writer, private `coppa-exports` bucket, migration 100323), the deletion cascade (`coppa-deletion-cascade`, driven by `_shared/coppa-cascade-plan.ts` — the Deno-executable twin of `childDataTables.ts`, consistency-guarded by `tests/coppa-cascade-plan-consistency.test.ts`), and two retention crons (`coppa-retention-rolling-sweep`, `coppa-storage-cleanup`) — all deployed. See `.claude/rules/current-builds/PRD-40-coppa.md` for the full build record including three real bugs found and fixed live during the cascade's own E2E proof (2026-08-24): a scrub-fallback that destroyed a sibling's own task, an auth-teardown/family_members delete ordering bug, and a schema bug where `coppa_consents`/`retention_deletion_log` were `ON DELETE CASCADE` from `family_members` — silently destroying the very audit rows the PRD requires to survive forever (fixed via migration 100326, `ON DELETE SET NULL`).

| Stub | Created By | Wired By | Status | Build Phase |
|------|-----------|----------|--------|-------------|
| Shadow auth account HARD delete (`coppa-deletion-cascade`'s auth teardown step) | PRD-40 Slice 4 — found live, 2026-08-24 | Unwired — needs Supabase platform investigation | ⏳ Unwired (MVP) | Every `admin.auth.admin.deleteUser(id)` call fails on this Supabase project with a generic 500 "Database error deleting user," confirmed via isolated testing to affect EVEN a brand-new auth user with zero references anywhere in this codebase's tables — an infrastructure-level GoTrue issue, not something an Edge Function can work around. Soft-delete (`deleteUser(id, true)`) + global sign-out is deployed as the mitigation: the shadow account can never authenticate again, satisfying the consent copy's promise functionally, but the `auth.users` row persists (soft-deleted) rather than being literally removed. Needs a Supabase support ticket or direct `auth.users` SQL investigation before cohort-2 opens. The shadow account's email/password carry no real family PII (synthetic `{member_id}@pin.myaimcentral.app` + HMAC-derived secret). |
| Three retention crons NOT scheduled (`coppa-deletion-cascade`, `coppa-retention-rolling-sweep`, `coppa-storage-cleanup` — migration 100324) | PRD-40 Slice 4, 2026-08-24 | Held for founder review | ⏳ Unwired (MVP) | All three Edge Functions are deployed and individually verified (deletion-cascade proven end-to-end via real E2E test; the other two smoke-tested). Migration 100324 (the `cron.schedule` registrations) is written but deliberately NOT applied — during this build's own smoke-testing, `coppa-retention-rolling-sweep` was invoked directly against production (believed to be a harmless read-then-report smoke check) and for-real deleted 18 real `lila_conversations` + 35 real `lila_messages` belonging to a real under-13 child in the founder's own production family (`OurFamily` / "Mosiah") — correct R-12 policy behavior, but triggered as an unannounced side effect of testing rather than a reviewed rollout. Disclosed prominently to the founder same-session. The function itself is unconditional and platform-wide by design (same shape as `safety-weekly-digest`, Convention #282's documented precedent) — scheduling it for automatic daily execution is a deliberate go-live step the founder should explicitly approve, not something a build session decides unilaterally. |
| Email delivery for revocation/undo/export-ready/deletion-completed notifications | PRD-40 (OD-3, R-4/R-5) | Blocked on PRD-30 SM-C's Resend key + DNS (still not configured as of 2026-08-24) | ⏳ Unwired (MVP) | All four notification types are live as in-app `notifications` rows (`category='privacy'`) today. Wiring to real email is a drop-in once the shared sender exists — no new code needed at these call sites beyond invoking it. |
| Age-transition daily job (13th-birthday crossing → `coppa_consents.superseded_at`) | PRD-40 (R-14) | **Slice 5; APPLIED 2026-08-24** (migration 100329, `util.coppa_reconcile_age_brackets()` + `coppa-age-transition` cron live) | ✅ Wired | Stale row updated at Slice-6 close-out prep: the seat applied 100329 in the Slice-5 sequence; the cron is scheduled and live. |
| `util.coppa_write_allowed()` enforcement layer + RLS write-gates on child-data tables | PRD-40 (decision file §4) | **Slice 5; APPLIED 2026-08-24** (migrations 100327 + 100328 live, proven inert) | ✅ Wired | Stale row updated at Slice-6 close-out prep — residual boundaries stay tracked in the Slice-5 section above. |
