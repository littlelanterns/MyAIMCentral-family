...
| FO Finances tab for finance-granted dads (kept mom-only this build, parity with prior Tasks-page gating; granted dads keep Prize Board) | FO-COMMAND-CENTER (2026-06-10) | Unwired | 📌 Unwired (Post-MVP) | Candidate: gate via `useManagementGrants.financeMaxLevel` |
| FO per-column long-press collapse override + section/column drag-reorder (PRD-14C spec; row-level collapse works) | PRD-14C original FO build (pre-existing gap, re-flagged by FO-COMMAND-CENTER 2026-06-10) | Unwired | ⏳ Unwired (MVP) | FO polish pass |
