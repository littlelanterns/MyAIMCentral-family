...
| Enforcement E2E full run + rls-verifier pass + tour | PRD-40 Slice 5 | Founder-gated proof sequence | ⏳ Unwired (MVP) | The spec (`tests/e2e/features/coppa-enforcement.spec.ts`, 9 tests incl. the INERTNESS before/after design), the transaction-scoped enforcement-active probe (`scripts/coppa-enforcement-probes.sql`, BEGIN…ROLLBACK, zero residue), and the Conv-#277 tour are all written; running them requires the founder-gated migration apply + suite slot + (for the AI-gate test) the lila-chat/bookshelf-discuss/lila-board-of-directors deploys. |
...
| Age-transition daily job (13th-birthday crossing → `coppa_consents.superseded_at`) | PRD-40 (R-14) | **BUILT by Slice 5** (migration 100329, `util.coppa_reconcile_age_brackets()` + `coppa-age-transition` cron) | ⏳ Written, apply founder-gated | Stale row updated 2026-08-24: the job exists; migration 100329 carries a first-run impact statement and can be applied or held independently of 100327/100328 (the 100324 precedent). |
| `util.coppa_write_allowed()` enforcement layer + RLS write-gates on child-data tables | PRD-40 (decision file §4) | **BUILT by Slice 5** (migrations 100327 + 100328) | ⏳ Written, apply founder-gated | Stale row updated 2026-08-24 — see the Slice 5 section above for the residual boundaries. |
