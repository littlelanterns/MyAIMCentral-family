...
| PRD-18 Phase B: `mindsweep_lite` placeholder | PRD-18 Phase B (Build K) | PRD-18 Phase C | ⏳ Unwired (MVP) | Phase C (Haiku disposition classification) |
| PRD-18 Phase B: `morning_insight` placeholder | PRD-18 Phase B (Build K) | PRD-18 Phase C | ⏳ Unwired (MVP) | Phase C (BookShelf semantic pull) |
| PRD-18 Phase B: `feature_discovery` placeholder | PRD-18 Phase B (Build K) | PRD-18 Phase C | ⏳ Unwired (MVP) | Phase C (activity-log nudge engine) |
| PRD-18 Phase B: `rhythm_tracker_prompts` auto-hide | PRD-18 Phase B (Build K) | PRD-18 Phase C | ⏳ Unwired (MVP) | Phase C (`dashboard_widgets.config.rhythm_keys`) |
...
| Routine duplication with linked step resolution | PRD-09A/09B Phase 1 | Linked Steps addendum (Build J → follow-up) | ⏳ Unwired (MVP) | Build J deferred. Schema + createTaskFromData ready. RoutineDuplicateDialog with per-linked-step source resolution prompt is a standalone follow-up — mom can manually rebuild routines for each child until then. |
...
| Per-item advancement override editor in SequentialCollectionView | Linked Steps addendum (Build J) | — | ⏳ Unwired (MVP) | Build J stubbed. Collection-level defaults propagate on create. Post-creation per-item override is a polish improvement — mom can recreate with different defaults if needed. |
