...
| Age-transition daily job (13th-birthday crossing → `coppa_consents.superseded_at`) | PRD-40 (R-14) | Not built | 📌 Unwired (Post-MVP) | Named in the PRD's incoming flows table; out of Slice 4's Rights + Lifecycle scope (revocation + export + deletion, not the aging-out transition). Screen 8's "Children Who Aged Out" section renders correctly once a row IS superseded (verified via direct DB seed in the eyes-on tour); nothing yet sets `superseded_at` automatically. |
| `util.coppa_write_allowed()` enforcement layer + RLS write-gates on child-data tables | PRD-40 (decision file §4) | Slice 5 | 📌 Unwired (Post-MVP within PRD-40, but tracked) | Explicitly out of Slice 4 scope per the dispatch ("Zero enforcement-gate work (Slice 5)"). |
